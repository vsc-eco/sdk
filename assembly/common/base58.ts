export namespace Base58 {
  ...
  // Base for Base58 encoding
  // @ts-ignore: decorator
  @lazy
  const BASE = 58;
  
  // Map of character codes to Base58 values
  // @ts-ignore: decorator
  @lazy
  const BASE_MAP = StaticArray.fromArray<u8>([
   ...
   * 
   * @param {Uint8Array} source - Byte array to be encoded.
   ...
   * @throws {Error} If the string contains a non-base58 character.
   * @example
   * ```ts
  ...
  function decodeUnsafe(source: string): Uint8Array {
      ...
      let size = i32((source.length - psz) * FACTOR + 1) >>> 0;
